...
import dev.xlin.gameworkshop.GUI.dialog.DlgBuildListData;
...
import dev.xlin.gameworkshop.progs.foundation.buildListData;
...
public class PalBuildDataList extends javax.swing.JPanel implements iAdtDocumentSave, iDatablockFace, iDatablockGUIPal
{

    private buildListData buildData = null;
    ...
    {
        myTreeNode mrt = guiDBLKPalCode.makeDatablockStructTree(buildData);
    ...
    {
        int r = guiDBLKPalCode.doNewPage(buildData);
        if (r >= 0)
        {
            makeTree();
        }
    ...
    {
        int r = guiDBLKPalCode.doNewCol(getSelectedPage(), buildData);
        if (r > 0)
        {
            makeTree();
        }
    ...
    {
        int r = guiDBLKPalCode.doEditPage(getSelectedPage(), buildData);
    ...
    private void doDelPage()
    {
        ...
        boolean b = guiDBLKPalCode.doDelPage(mpage, buildData);
        if (b)
        {
            makeTree();
        }
    ...
    private void doEditCol()
    {
        ...
        int r = guiDBLKPalCode.doEditCol(mcol, buildData);
        if (r == iDAO.OPERATE_SUCCESS)
        {
            makeTree();
        }
    ...
    {
        int r = guiDBLKPalCode.doDelCol(getSelectedCol(), buildData);
        if (r == iDAO.OPERATE_SUCCESS)
        {
            makeTree();
        }
    ...
    private void doMovePageUp()
    {
        ...
        boolean b = guiDBLKPalCode.doMovePageUp(mtn, buildData);
        if (b)
        {
            makeTree();
        }
    ...
    {
        myTreeNode mtn = getSelectedPage();
        boolean b = guiDBLKPalCode.doMovePageDown(mtn, buildData);
        if (b)
        {
            makeTree();
        }
    ...
    {
        boolean b = guiDBLKPalCode.doMoveColUp(getSelectedCol(), buildData);
        if (b)
        {
            makeTree();
        }
    ...
    {
        boolean b = guiDBLKPalCode.doMoveDataUp(getSelectedDataElement(), buildData);
        if (b)
        {
            makeTable();
        }
    ...
    {
        boolean b = guiDBLKPalCode.doMoveDataDown(getSelectedDataElement(), buildData);
        if (b)
        {
            makeTable();
        }
    ...
    {
        boolean b = guiDBLKPalCode.doMoveDataToCol(getSelectedDataElement(), buildData);
        if (b)
        {
            makeTable();
        }
    ...
    {
        boolean b = guiDBLKPalCode.doMoveColToPage(getSelectedCol(), buildData);
        if (b)
        {
            makeTree();
        }
    ...
    private void doDisData()
    {
        ...
        int r = guiDBLKPalCode.disData(ide, buildData);
        if (r == iDAO.OPERATE_SUCCESS)
        {
            makeTable();
        }
    ...
    {
        iDataElement ide = getSelectedDataElement();
        int r = guiDBLKPalCode.revertData(ide, buildData);
        if (r == iDAO.OPERATE_SUCCESS)
        {
            makeTable();
        }
    ...
    {
        iDataElement ide = getSelectedDataElement();
        int r = guiDBLKPalCode.destroyData(ide, buildData);
        if (r == iDAO.OPERATE_SUCCESS)
        {
            makeTable();
        }
    ...
    {
        int r = guiDBLKPalCode.guiSaveAsTemplet(up, oclsID, buildData, dbIndex);
        if (r == iDAO.OPERATE_SUCCESS)
        {
    ...
    {
        boolean b = guiDBLKPalCode.guiImportTemplet(up, buildData);
        if (b)
        {
            makeTree();
            makeTable();
    ...
    private void doSave(boolean showMsg)
    {
        ...
        xmlRight xr = buildData.transToXML();
    ...
    private void makeTable()
    {
        ...
        if (mcol != null)
        {
            ldatas = buildData.getDataElementList(mcol.getNodeOID(), ckShowAll.isSelected());
            if (ldatas != null)
            {
                for (int i = 0; i < ldatas.size(); i++)
                {
                    BeanBuildListItemData bean = (BeanBuildListItemData) ldatas.get(i);
                    Vector v = new Vector();
    ...
    private void doNewData()
    {
        ...
        DlgBuildListData dlg = new DlgBuildListData(null, true, up, null);
        dlg.setVisible(true);
        if (dlg.getOK())
        {
            BeanBuildListItemData bean = dlg.getBuildListItemData();
            int r = buildData.appendDataElement(bean, mcol.getNodeOID());
            if (r == iDAO.OPERATE_SUCCESS)
            {
                makeTable();
            }
    ...
    private void doEditData()
    {
        ...
        DlgBuildListData dlg = new DlgBuildListData(null, true, up, (BeanBuildListItemData) ide);
        dlg.setVisible(true);
        if (dlg.getOK())
        {
            BeanBuildListItemData bean = dlg.getBuildListItemData();
            int r = buildData.updateDataElement(bean);
            if (r == iDAO.OPERATE_SUCCESS)
            {
                makeTable();
            }
    ...
    private void initComponents()
    {
        ...
        btnPopBuild = new javax.swing.JButton();
        ...
        miNewDataFromCol.setText("添加建造数据");
        ...
        btnPopBuild.setForeground(new java.awt.Color(0, 0, 204));
        btnPopBuild.setText("↓");
        btnPopBuild.setFocusable(false);
        btnPopBuild.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
        btnPopBuild.setVerticalTextPosition(javax.swing.SwingConstants.BOTTOM);
        btnPopBuild.addMouseListener(new java.awt.event.MouseAdapter()
        {
            public void mouseClicked(java.awt.event.MouseEvent evt)
            {
                btnPopBuildMouseClicked(evt);
            }
        });
        btnPopBuild.addActionListener(new java.awt.event.ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                btnPopBuildActionPerformed(evt);
            }
        });
        jToolBar3.add(btnPopBuild);
    ...
    private void btnPopBuildMouseClicked(java.awt.event.MouseEvent evt)//GEN-FIRST:event_btnPopBuildMouseClicked
    {//GEN-HEADEREND:event_btnPopBuildMouseClicked
        if (evt.getButton() == evt.BUTTON1)
        {
            popDatas.show(btnPopBuild, evt.getX(), popDatas.getY() + btnPopBuild.getHeight());
        }
    }//GEN-LAST:event_btnPopBuildMouseClicked

    private void btnPopBuildActionPerformed(java.awt.event.ActionEvent evt)//GEN-FIRST:event_btnPopBuildActionPerformed
    {//GEN-HEADEREND:event_btnPopBuildActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_btnPopBuildActionPerformed


    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JButton btnImportTemplet;
    private javax.swing.JButton btnPopBuild;
    ...
    {
        return buildData.getDatablockServiceTag();
    }

    @Override
    public String getDatablockName()
    {
        return buildData.getDatablockName();
    }
    ...
    public void initData(wakeup _up, long _dbOID, int _oclsID)
    {
        ...
        if (oclsID == 0)
        {
            btnSaveAsTemplet.setEnabled(false);
            btnImportTemplet.setEnabled(false);
        }
        buildData = new buildListData();
        buildData.initDatablcok(up);
        initDatablock();
        makeTree();
        makeTable();
    }
    ...
    private void initDatablock()
    {
        ...
        if (b)
        {
            boolean b2 = buildData.revertFromXML(xr);
        }
        else
        {
        }
